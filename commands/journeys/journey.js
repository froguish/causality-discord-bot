...
async function createJourney(ctx, a, b, c, ping){
	...
	const conversationLog = [
		{ role: 'system', content: `You are a scenario describing storyteller. Your name is Causality. You describe gritty and realistic scenarios. The elements provided are the player: (${playerInfo[0]}), the player's goal: (${playerInfo[1]}), and the setting: (${playerInfo[2]}).`}, { role: 'user', content: `Given those elements, you are to generate an introduction of the character, their goal, and their setting. Finish the generation with "What will you do?"`}]
